...
#     try:
#         response = requests.post(address, data=data, headers=headers, timeout=60, verify=False)
...
# def send_request(type='GET', url='', data=None, expect_code=200, timeout=60, func=''):
#     # check params
...
#     if type == 'PATCH':
#         headers = {"Authorization": token_bear, "Content-Type": "application/json"}
#         response = requests.patch(url, headers=headers, json=data, timeout=timeout, verify=False)
#     # check response code
#     if response.status_code != expect_code:
#         print('Response failed at: ', func)
#         exit(1)
#     return response
...
class VnfPkgs(object):
    def __init__(self):
        self.vnfpkg_baseload = sig_data_dir + r'\Nokia_sig_SBC-VNF_Package.zip'
        self.vnfpkg_drbaseload = sig_data_dir + r'\Nokia_sig_SBC-VNF_Package-DR.zip'
        self.vnfpkg_sutoload = sig_data_dir + r'\Nokia_sig_SBC-VNF_Package-SUToLoad.zip'
        self.vnfpkg_cssutoload = sig_data_dir + r'\Nokia_sig_SBC-VNF_Package-CSSUToLoad.zip'
        self.vnfpkgs_list = []
    ...
    def upload_vnfpkg(self, version = '37.34.06', type='instantiation'):
        vnfpkg = ''
        id = ''
        if type not in ['instantiation', 'su', 'dr', 'cssu']:
            logger.info('vnfpkg type not supported. Need to be one of ' +
                        str(['instantiation', 'su', 'dr', 'cssu']))
...
class SigVnf(object):
    def __init__(self, vnfdId, name='sbclcm-sig-plane',
        ...
        self.dr_instantiate_file = sig_data_dir + r'\dr_LCM_instantiate_params.json'
...
class SigVnfArtsGenerator(object):
    def __init__(self, type='instantiation', rel='R20.2'):
        self.type = type
        self.rel = rel
        # self.torel = torel
    ...
    # def prep_dr_instantiation_json(self):
    ...
    #
    #     try:
    #         with open(self.dr_instantiation_json_file, 'w') as jFile:
    #             json.dump(data, jFile, indent=2)
    #     except Exception as exc:
    #         print('ERROR: Failed to write data to {0}.\n[{1}]'.format(
    #             self.dr_instantiation_json_file, str(exc)))
    #
    #     # copy the dr instantiation to data\sig-plane
    #     logger.info('copy ' + self.dr_instantiation_json_file)
    #     srcfile = self.dr_instantiation_json_file
    #     dstfile = sig_data_dir + '\\' + self.dr_instantiation_json_file
    #     shutil.copyfile(srcfile, dstfile)
...
def sigvnf_GenArtsInstantiation():
    sigArts = SigVnfArtsGenerator(type='instantiation', rel='R20.2')
    sigArts.prep_arts()

def sigvnf_GenArtsDR():
    sigArts = SigVnfArtsGenerator(type='dr', rel='R20.2')
    sigArts.prep_arts()

def sigvnf_GenArtsSU():
    sigArts = SigVnfArtsGenerator(type='su', rel='R20.2')
    sigArts.prep_arts()

def sigvnf_GenArtsCSSU():
    sigArts = SigVnfArtsGenerator(type='cssu', rel='R20.2')
    sigArts.prep_arts()
...
class LcmTestDriver(object):
    ...
    # rel is current release
    def sigvnf_GenArts4DR(self, rel='R20.2'):
        logger.info('In Func: ' + sys._getframe().f_code.co_name)
        self.setup_sigDriver()
        current_count, max_count = self.sigDriver.get_scale_status()
    ...
    def sigvnf_GenArts4SU(self, rel='R20.2'):
        logger.info('In Func: ' + sys._getframe().f_code.co_name)
        self.setup_sigDriver()
        current_count, max_count = self.sigDriver.get_scale_status()
    ...
    # rel is TO load release
    def sigvnf_GenArts4CSSU(self, rel='R20.2'):
        logger.info('In Func: ' + sys._getframe().f_code.co_name)
        self.setup_sigDriver()
        current_count, max_count = self.sigDriver.get_scale_status()
    ...
    def sigvnf_GenArtsTests(self, rel='R20.2'):
        logger.info('In Func: ' + sys._getframe().f_code.co_name)
        self.setup_sigDriver()
    ...
    def sigvnf_tests_gamma(self):
        ...
        self.sigvnf_Backup()
        self.sigvnf_DBRestore()
        self.sigvnf_GenArts4DR(rel='R20.2')
    ...
    def sigvnf_tests_dr_v2(self):
        self.sigvnf_Backup_Remote_Creds1()
        self.sigvnf_GenArts4DR()
        self.sigvnf_tests_td()
        sigvnf_UploadVnfpkg(swVersion=sigVersion, type='dr')
        setup_vnfdIds()
        self.refresh_sigDriver(sig_vnfdId)
    ...
    def sigvnf_tests_cssu_v2(self):
        self.sigvnf_Backup_Remote_Creds1()
        self.sigvnf_GenArts4CSSU(rel='R20.2')
    ...
    def sigvnf_tests_su_v2(self):
        self.sigvnf_GenArts4SU(rel='R20.2')
...
def TS_sigvnf_tests_su():
    lcmDriver = LcmTestDriver(sig_vnfdId)
    lcmDriver.sigvnf_GenArts4SU(rel='R20.2')
    sigvnf_UploadVnfpkg(swVersion=sigVersion_SU, type='su')
    setup_vnfdIds()
    lcmDriver = LcmTestDriver(sig_vnfdId)
...
def TS_sigvnf_tests_cssu():
    lcmDriver = LcmTestDriver(sig_vnfdId)
    lcmDriver.sigvnf_GenArts4CSSU(rel='R20.2')
...
def TS_sigvnf_tests_dr():
    ...
    lcmDriver.sigvnf_GenArts4DR(rel='R20.2')
    lcmDriver.sigvnf_tests_td()
    sigvnf_UploadVnfpkg(swVersion=sigVersion, type='dr')
    setup_vnfdIds()
    lcmDriver = LcmTestDriver(sig_vnfdId)
...
def TS_sigvnf_tests_dr_v2():
    lcmDriver = LcmTestDriver(sig_vnfdId)
    lcmDriver.sigvnf_tests_dr_v2()
