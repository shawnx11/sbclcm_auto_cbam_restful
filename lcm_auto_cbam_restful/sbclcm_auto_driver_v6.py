...
mediaVersion = 'ap100009'
mediaVersion_SU = 'ap100010'
...
class VnfPkgs(object):
    ...
    def upload_vnfpkg(self, sig_media='sig', version = sigVersion, type='instantiation'):
        ...
        if sig_media not in ['sig', 'media']:
            logger.info('upload_vnfpkg: need to be either sig or media.')
            exit(1)
        if sig_media == 'sig':
...
class MediaVnf(object):
    def __init__(self, vnfdId, apiVersion='3', name='sbclcm-media-plane',
        ...
        self.vnflcm_instances = self.vnflcm_base_path + '/vnf_instances/'
        self.vnfpkgm_base_path = cbam_url + '/vnfpkgm/v1'
        self.vnflcm_create = self.vnflcm_base_path + '/vnf_instances'

    def set_vnflcm_apis(self):
        if self.apiVersion == '4':
            self.vnflcm_base_path           = cbam_url + '/vnflcm/v1'
            self.vnflcm_instances           = self.vnflcm_base_path + '/vnf_instances/'
            self.operationState             = self.vnflcm_base_path + '/vnf_lcm_op_occs'
            self.vnflcm_create              = self.vnflcm_base_path + '/vnf_instances'
        if self.apiVersion == '3':
            self.vnflcm_base_path = cbam_url + '/vnfm/lcm/v3'
            self.vnflcm_instances = self.vnflcm_base_path + '/vnfs/'
            self.operationState = self.vnflcm_base_path + '/operation_executions'
            self.vnflcm_create = self.vnflcm_base_path + '/vnfs'
        # For operation state, v4 version is good for both v3 and v4
    ...
    # url_v3 = 'https://100.69.127.147/vnfm/lcm/v3/vnfs/CBAM-5a78b2b2f4a04e7f9b403a5e57d5e2a9/instantiate'
    def set_vnflcm_apis_v3(self):
        self.vnflcm_base_path           = cbam_url + '/vnfm/lcm/v3'
        self.vnflcm_instances           = self.vnflcm_base_path + '/vnfs/'
        self.operationState             = self.vnflcm_base_path + '/operation_executions'
        self.vnflcm_create              = self.vnflcm_base_path + '/vnfs'
    ...
    #########################################################
    # Following are for combined V3 and V4 API
    #########################################################
    def create_vnf(self):
        data = {
            "vnfdId": self.vnfdId,
    ...
    #   "vims": [
    #     {
    #       "interfaceEndpoint": "https://10.75.12.196:13000",
    #       "accessInfo": {
    #         "tenant": "CB0078",
    #         "username": "cb0078sa",
    #         "password": "LabAcc0unt"
    ...
    def heal_vnf(self, vnfcInstanceId=None, cause='Media Plane VM Heal: SCM'):
        if vnfcInstanceId is None:
            vnfcInstanceId = ['OAM.NOKIA-LCP-VMA']
        data = {
            "cause": cause,
    ...
    def scale_vnf(self, scaleType='SCALE_OUT', step=1, aspectId="pim_Aspect"):
        data = {
            "type": scaleType,
            "aspectId": aspectId,
            "numberOfSteps": step
    ...
    #########################################################
    # Following are for V4 API
    #########################################################
    def create_vnf_v4(self):
        data = {
            "vnfdId": self.vnfdId,
            "vnfInstanceName": self.name,
            "vnfInstanceDescription": self.description
        ...
        self.id = data['id']
        logger.info('create_vnf:id: '+ self.get_id())
        self.set_vnflcm_apis_v4()

    #MODIFY use PATCH
    def modify_extension_vnf_v4(self):
        data_extension = json.load(open(self.extension_file, "rb"))
        # need to reformat data to VnfInfoModificationRequest format
        data_reformatted = {'extensions': {}}
        for nv in data_extension['extensions']:
            dict_tmp = {}
            dict_tmp[nv['name']] = nv['value']
            data_reformatted['extensions'].update(dict_tmp)
        response = send_request(type='PATCH', url=self.vnflcm_modify, data=data_reformatted,
                                expect_code=202, func=sys._getframe().f_code.co_name)

    def modify_auto_scale_vnf_v4(self, periodInSeconds=600, enabled=False):
        data = {
            "vnfConfigurableProperties": {
                "operation_triggers": {
                    "auto_scale": {
    ...
    # For now, v4 not working
    def instantiate_vnf_v4(self):
        data = json.load(open(self.instantiate_file, "rb"))
        response = send_request(type='POST', url=self.vnflcm_instantiate, data=data,
                                expect_code=202, func=sys._getframe().f_code.co_name)
...
class MediaVnfLcmTestDriver(object):
    def __init__(self, medvnf):
        # Need to determine which is media plane vnf
        # Need to get all vnfs and decide
        self.getvnfs = cbam_url + '/vnflcm/v1' + '/vnf_instances/'
        self.medvnf = medvnf
        self.media_id = ''
        self.media_name = 'sbgw01'
        self.sig_oama_ip = '100.69.127.133'
        self.sig_oam_login = 'root'
        self.sig_oam_passwd = 'newsys'
        ...
        self.su_to_image = 'nokia-SBC_sig-RHEL7-R37.28.06.0020.x86_64.qcow2'
        self.backup_location = 'http://100.69.127.146/sbclcm-auto/BACKUP_C.ZIP'
        # Following are additionalParams for various operations
        # For backup:
        self.additinalParams_Backup_Local = {
            'backup_server1': '',
    ...
    def restore_vnf(self):
        # This is to restore info of media VNF
        self.media_id = self.get_vnf_id()
        self.medvnf.set_id(self.media_id)
        self.medvnf.set_vnflcm_apis()
    ...
    def get_vnf_id(self):
        ...
        if response.status_code == 200:
            ...
            for vnf in data:
                # Fow now, use vnfProductName to determine if this is sig plane vnf
                if vnf['vnfProductName'] == 'SBC-media':
                    logger.info('get_vnf_id: media vnf id: ' + vnf['id'])
                    return vnf['id']
        else:
            logger.info('Failed to get_vnf_id')
            exit(1)
    ...
    def create_instantiate(self):
        self.medvnf.create_vnf()
        self.medvnf.modify_extension_vnf()
        self.medvnf.instantiate_vnf()
        self.medvnf.get_vnf()

    def terminate(self):
        self.medvnf.terminate_vnf()

    def delete(self):
        self.medvnf.delete_vnf()

    def change_package_version(self, vnfdId):
        self.medvnf.upgrade_vnf(vnfdId)

    def get_vnfcInstanceId_List_4Heal(self):
        self.medvnf.get_vnfcInstanceId_List()
        return self.medvnf.vnfcInstanceId_List

    def heal(self, vnfcInstanceId=None, cause='Media Plane VM Heal: SCM'):
        if vnfcInstanceId is None:
            vnfcInstanceId = ['OAM.NOKIA-LCP-VMA']
        self.medvnf.heal_vnf(vnfcInstanceId, cause)

    # def get_scale_status(self):
    #     self.medvnf.get_scaleStatus()
    #     return self.sigvnf.scaleLevel, self.sigvnf.maxScaleLevel
    #
    # def scale(self, scaleType='SCALE_OUT', step=1, aspectId="sc_Aspect"):
    #     self.sigvnf.get_scaleStatus()
    ...
    def custom_backup(self, additionalParam):
        self.medvnf.backup_vnf(additionalParam)
    ...
    def check_latestOpState(self, timeout=30):
        status = self.medvnf.get_latestOperationState()
        if status is not None:
            return status
    ...
    def check_opstatus(self, operation='INSTANTIATE', operationName='', timeout=30):
        wait = 0
        while wait < timeout:
            status = self.medvnf.get_operationState(operation, operationName)
            if status == 'PROCESSING':
                logger.info('Media Plane VNF ' + operation + ' ' + operationName + ' PROCESSING')
                logger.info('Wait for 60 secs...')
            elif status == 'FAILED':
                logger.info('Media Plane VNF ' + operation + ' ' + operationName + ' FAILED')
                # log the detailed error info
                data = self.medvnf.get_latestOperation()
                logger.info(str(data))
                exit(1)
            elif status == 'ROLLED_BACK ':
                logger.info('Media Plane VNF ' + operation + ' ' + operationName + ' ROLLED_BACK')
                exit(2)
            elif status == 'COMPLETED':
                logger.info('Media Plane VNF ' + operation + ' ' + operationName + ' COMPLETED')
                return 0
            else:
                logger.info('Media Plane VNF ' + operation + ' ' + operationName + ' status: ' + status)
            # Check status every 60 secs
            time.sleep(60)
            wait = wait + 1
